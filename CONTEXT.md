...
| **Registry** | `src/core/module/registry.ts` — the one file every module team shares. Navigation, gated paths, section lookups, meta validation, message merging, redirects, the CSP's `frame-src`, the landing page's bento tiles and the crawl rules behind `robots.txt` / `sitemap.xml` are all *derived* from it (in `derive.ts`, and for the landing's layout arithmetic in `src/components/landing/tiles.ts`); nothing else keeps a per-module list. |
| **Landing tile** | What a section contributes to the landing bento, declared as data on its manifest (`landingTile`) and rendered by core — the same split as `metaFields` and the admin form. Three kinds: `list`, `stat`, `fields`. A module never declares its own width; span follows kind, and a tile is suppressed while its section has nothing published. Opt-in: a section earns a tile by having something to show (ADR 0015). |
...
| **Gating tiers** | (1) Public: landing, module indexes, section listings and `/search` (title/summary). (2) Signed-in: resource detail pages. (3) Admin: `/admin`, which 404s for non-admins rather than revealing it exists. A module declares which of its paths are gated; the patterns are derived, not hand-listed. Separately from gating, a path can be public-but-uncrawlable (`platformNoIndexPaths`): `/search` is readable by anyone but kept out of search engines, because a query string is an unbounded URL space (ADR 0016). |
