...
| **Registry** | `src/core/module/registry.ts` — the one file every module team shares. Navigation, gated paths, section lookups, meta validation, message merging, redirects, the CSP's `frame-src` and the crawl rules behind `robots.txt` / `sitemap.xml` are all *derived* from it in `derive.ts`; nothing else keeps a per-module list. |
