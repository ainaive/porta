...
`bun dev`, `bun test`, seed scripts). Production runs Node 24 (the active
LTS, and Vercel's default) in both targets. App code uses no Bun-only
APIs. Because Next 16's standalone output bundles server dependencies
into chunks (leaving no node_modules for external scripts), the migration
script is bundled into a self-contained file with `bun build --target=node`
and executed by the container entrypoint before the server starts; Vercel
runs the same script from node_modules during its build.
